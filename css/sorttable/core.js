...
jQuery.fn.showpagination = function (rowperpage, divid) {
    ...
    if (rows > rowperpage) {
        ...
        jQuery(tid).after('<div class="pagination ' + id + '" idd="' + id
            + '"></div>');
        jQuery(current).append('<div class="pag-info">Hiển thị: ' + rowperpage + ' | Tổng số: ' + rows + '</div>');
        jQuery(current)
            .append('<div id="pag-btn"><p class="page-list"></p></div>');
        for (var x = 1; x < 4; x++) {
            if (x > max) {
                break;
            }
        ...
        jQuery('.page-item').live('click', function () {
            ...
            if (page != 0) {
                ...
                jQuery('.pag-info', '.' + idd).html('Hiển thị: '
